defmodule Packer.Reporter do
  ...
  def run(instance, %{"processes_on_node" => processes} = solution) do
    %{
      ...
      process_demand:
        Map.take(instance, [:process_memory, :process_load, :process_message_volume])
    }
  end
  ...
  @spec markdown_report(map(), map()) :: <<_::64, _::_*8>>
  ...
  defp feasible_mapping_markdown(%{node_statistics: statistics} = _report) do
    Enum.reduce(statistics, "", fn {node_id,
                                   ...
                                   acc ->
      acc <>
        "|  #{node_id}| {#{Enum.join(processes, ",")}} | #{memory}/#{memory_available} | #{load}/#{load_available} | #{traffic_out}/#{bandwidth_out} | #{traffic_in}/#{bandwidth_in} \n"
    end)
  end
  ...
  defp topology_markdown(%{topology: topology} = _report) do
    for i <- 1..length(topology), reduce: "" do
      acc ->
        line =
          for j <- 1..length(topology), reduce: "| Node #{i}" do
            line_acc ->
              line_acc <>
                "| " <>
                if i == j do
